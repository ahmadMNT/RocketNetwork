...
class AuthTokenManagerTests: XCTestCase {
    ...
    func testGetAccessToken_WithValidToken() {
        ...
        XCTAssertEqual(mockApiClient.refreshCallCount, 0, "Should not attempt to refresh a valid token")
    }
    ...
    func testGetAccessToken_WithNoToken_RefreshSuccess() {
        ...
        XCTAssertNil(resultError)
        XCTAssertEqual(mockApiClient.refreshCallCount, 1, "Should attempt to refresh when no access token")
    ...
    func testGetAccessToken_WithNoToken_NoRefreshToken() {
        ...
        XCTAssertEqual(resultError as? NetworkError, NetworkError.unauthenticated, "Should return unauthorized when no tokens")
        XCTAssertEqual(mockApiClient.refreshCallCount, 0, "Should not attempt to refresh when no refresh token")
    }
    ...
    func testGetAccessToken_WithNoToken_RefreshFailure() {
        ...
        XCTAssertNil(resultToken)
        XCTAssertNotNil(resultError)
        XCTAssertEqual((resultError as? NetworkError)?.localizedDescription, refreshError.localizedDescription, "Should return refresh error")
    ...
    func testRefreshToken_Success() {
        ...
        XCTAssertEqual(mockApiClient.lastRefreshToken, refreshToken, "Should use stored refresh token")
        XCTAssertEqual(mockStorage.accessToken, newAccessToken, "Should store new access token")
        XCTAssertEqual(mockStorage.refreshToken, newRefreshToken, "Should store new refresh token")
    }
    ...
    func testRefreshToken_NoRefreshToken() {
        ...
        XCTAssertNotNil(resultError)
        XCTAssertEqual(resultError as? NetworkError, NetworkError.unauthenticated, "Should return unauthorized when no refresh token")
        XCTAssertEqual(mockApiClient.refreshCallCount, 0, "Should not call API with no refresh token")
    }
    ...
    func testRefreshToken_ApiFailure() {
        ...
        XCTAssertFalse(resultSuccess)
        XCTAssertNotNil(resultError)
        XCTAssertEqual((resultError as? NetworkError)?.localizedDescription, refreshError.localizedDescription, "Should return API error")
        XCTAssertEqual(mockApiClient.refreshCallCount, 1, "Should call refresh API")
    }
