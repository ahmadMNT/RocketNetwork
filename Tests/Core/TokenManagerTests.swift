...
class TokenManagerTests: XCTestCase {
    ...
    func testSaveAndRetrieveAccessToken() {
        ...
        XCTAssertEqual(mockUserDefaults.savedValues[TokenManager.Constants.accessTokenKey] as? String, testToken)
    }
    ...
    func testSaveAndRetrieveRefreshToken() {
        ...
        // Then
        XCTAssertEqual(retrievedToken, testToken)
        XCTAssertEqual(mockUserDefaults.savedValues[TokenManager.Constants.refreshTokenKey] as? String, testToken)
    }
    ...
    func testRefreshToken_Success() {
        ...
        // When
        tokenManager.refreshToken(using: mockSession, with: URL(string: "https://api.example.com/refresh")!) { result in
            // Then
            switch result {
            case .success:
        ...
        XCTAssertEqual(mockSession.lastRequest?.url?.absoluteString, "https://api.example.com/refresh")
