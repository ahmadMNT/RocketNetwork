...
final class APIEndpointTests: XCTestCase {
    ...
    struct CustomizedMockEndpoint: APIEndpoint {
        ...
        var queryParameters: [URLQueryItem] {
            return [URLQueryItem(name: "custom", value: "param")]
        }
    ...
    func testBuildURLRequest() {
        ...
        // Check URL
        XCTAssertEqual(
            request.url?.absoluteString, "https://api.example.com/test/endpoint?test=value")
    ...
    func testBuildURLRequestWithCustomization() {
        ...
        // Check URL
        XCTAssertEqual(
            request.url?.absoluteString, "http://test.example.com:8080/custom/path?custom=param")
    ...
    private func getAuthorizationHeaders(for credentials: AuthenticationCredentials) -> [String:
        String]?
    {
