...
final class NetworkErrorTests: XCTestCase {
    ...
    func testEquality() {
        ...
        XCTAssertEqual(
            NetworkError.serverError(statusCode: 500), NetworkError.serverError(statusCode: 500))

        // Test different error types are not equal
        XCTAssertNotEqual(NetworkError.invalidResponse, NetworkError.unauthenticated)
        XCTAssertNotEqual(
            NetworkError.serverError(statusCode: 500), NetworkError.serverError(statusCode: 404))
        XCTAssertNotEqual(
            NetworkError.serverMessage(message: "Error 1"),
            NetworkError.serverMessage(message: "Error 2")
        )
